...
#### STAGE 2: DIMENSIONALITY REDUCTION ISNTANCE METHOD ####################################################################################################################
    def dimreduction(self, n_pcs = 100, pc_selection_method = "screeplot",  n_neighbors=15, min_dist = 0.1,
                  use_highly_variable = False,variance_threshold = 90, verbose = True,plot_result = False,color = "total_counts"):
      """
      ...
      def _plot():
        """plot the results"""
        if self.method == "PCA":
          sc.pl.pca(self.adata,color=color,add_outline = True, size = 100, title= "PCA",show=False)
          ...
          sc.pl.tsne(self.adata,add_outline = True, size = 100,title = "t-SNE",color=color)
        elif self.method  == "UMAP":
          sc.pl.umap(self.adata, add_outline=True, size=100, title="UMAP", color=color)
